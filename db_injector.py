...
def init_firebase():
    try:
        if not firebase_admin._apps:
            # Pastikan file firebase_key.json ditaruh sejajar di folder project lu
...
def tarik_data_api(emiten, tahun, periode):
    try:
        res = requests.get(f"http://127.0.0.1:8000/api/v1/laporan/{emiten}/{tahun}/{periode}", timeout=15)
...
if tombol_injeksi:
    if db is None:
        st.error("❌ Firebase SDK gagal inisialisasi. Periksa kunci `firebase_key.json` lu!")
        ...
        for idx, emiten_full in enumerate(emiten_massal):
            ...
            if res_api and res_api.get("hasil", {}).get("status") == "sukses":
                ...
                # Setup Struktur Firestore Document ID & Payload
                ...
                payload = {
                    ...
                    "periode": periode_api,
                    "total_aset": keuangan_data["Total_Aset"],
                    "laba_bersih": keuangan_data["Laba_Rugi_Bersih"],
                    "pendapatan": keuangan_data["Pendapatan"],
                    "metode_ekstraksi": metode,
                    "sinkronisasi_timestamp": firestore.SERVER_TIMESTAMP
                }
                
                try:
                    # Delay kecil 0.8 detik biar animasinya keliatan mengalir di layar
                    time.sleep(0.8)
                    
                    # Tembak masuk ke Firestore Collection
                    ...
                    data_sukses_injeksi.append({
                        ...
                        "Aset": keuangan_data["Total_Aset"], "Laba": keuangan_data["Laba_Rugi_Bersih"], "Status": "Saved"
                    })
                except Exception as ex:
                    riwayat_log.append(f"[{time.strftime('%H:%M:%S')}] ❌ ERROR FIREBASE pada emiten {ticker}: {ex}")
            else:
                riwayat_log[-1] = f"[{time.strftime('%H:%M:%S')}] ❌ Ekstraksi {ticker} GAGAL. Data tidak ditemukan di server IDX."
        ...
        # Tampilkan ringkasan data yang baru masuk
